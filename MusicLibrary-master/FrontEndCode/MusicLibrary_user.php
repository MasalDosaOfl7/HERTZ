...
<head>
    ...
    <div id="left">
        <p><b>Profile summary: </b><br/><i><?php GetUserSummary();?> </i></p>
    </div>
            ...
            <form id="mainform" name="mainform" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" >
                <table>
                    ...
                    <tr><td><select id="choice" name="choice" onchange="selectChoice(this.value)">
                        ...
                        <option value="display_allsongs" selected="selected">Display All Songs</option>
                        <option value="search_song">Search Song</option>
                        <!-- <option value="advsearch_song">Advanced Search Song</option> -->
                        <option value="view_pl">Playlists</option>
                        <option value="view_likes">Liked Songs</option>
                        <option value="view_history">Recently Played</option>
                    </select></td></tr>
                 </table>
                 <!--Search container that has the search field enclosed which will be visible various user operations when user performs search-->
                    <div id="Search_Container" style="display:None"> 
                        <table>
                        <tr><td><label class="description" for="element_2"><u>Search</u></label></td></tr>
...
<?php
    ...
    function table_songs($result){
        ...
        echo "<table class='table table-sm' border = '1px solid black' id='results_table' name='results_table'>";
        echo "<tr>";
        echo "<th><b>S.No</b></th>";
        ...
        while ($row=mysqli_fetch_array($result)) {
            ...
            echo "<td><table><tr><td><audio class='audio' src='".$row['Title'].".mp3'></audio><a class='play-pause' title='play/pause' type='button' onclick='PlaySong(".$row['SongID'] . "," . $row_count ." )' style='background-color:transparent; border-color:transparent;'><i class='fa fa-play'></i></a></td>";
    ...
    //Function to display all playlist in a table
     function table_pl($result){
        ...
        $row_count = 1;
        echo "<table class='table table-sm' border = '1px solid black' id='pl_results_table' name='pl_results_table'>";
        echo "<tr>";
        echo "<th><b>S.No</b></td>";
        ...
        echo  "<td>" . $row['playlist_name'] . "</td>";
         echo "<td><button title='Edit Playlist' type='button' onclick='EditPlaylist(".$row['playlist_ID']." );' style='background-color:transparent; border-color:transparent;'><img src ='https://cdn0.iconfinder.com/data/icons/glyphpack/45/edit-alt-512.png' height=25px width=25px></button>";
    ...
    function table_history($result){
        ...
        $row_count = 1;
        echo "<table class='table table-sm' border = '1px solid black' id='table_history' name='table_history'>";
        echo "<tr>";
        echo "<th><b>S.No</b></td>";
